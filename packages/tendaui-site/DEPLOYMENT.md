...
如果遇到 `ERR_PNPM_OUTDATED_LOCKFILE` 错误，说明 `pnpm-lock.yaml` 与 `package.json` 不同步。
...
当前 workflow 已配置为使用 `--no-frozen-lockfile`，允许在 CI 中自动更新 lockfile。但这会降低构建的可重复性，建议尽快在本地更新并提交 lockfile。
