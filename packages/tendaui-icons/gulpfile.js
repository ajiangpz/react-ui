...
gulp.task("moveScss", function moveScss() {
  return gulp.src(["src/**/*.scss"]).pipe(gulp.dest("lib/cjs")).pipe(gulp.dest("lib/es"));
});

gulp.task(
  "compileLib",
  gulp.series(["cleanLib", "compileScss", "moveScss", gulp.parallel("compileTSXForESM", "compileTSXForCJS")])
);
