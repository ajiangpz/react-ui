...
- 协商缓存服务端返回 last-modified 和 etag；客户端请求：If-Modified-Since 和 If-None-Match；
- 如果资源没有变化，则返回304，否则返回200；
- 优先级：ETag > Last-Modified 即 If-None-Match > If-Modified-Since；
---

#### DNS解析
- 递归查询： 浏览器-本地DNS缓存-本地hosts文件-本地DNS服务器；
- 迭代查询： 本地DNS-根域名服务器
          本地DNS-顶级域名服务器
          本地DNS-权威域名服务器
---

例如访问 www.baidu.com 的流程：
1. 电脑向本地 DNS：递归查询
2. 本地 DNS 向根服务器：迭代
3. 根 → 给出 .com 服务器地址
4. 本地 DNS 向 .com：迭代
5. .com → 返回 baidu.com 权威服务器地址
6. 本地 DNS → 权威 DNS：迭代
7. 最终得到 IP
8. 本地 DNS 把结果返回给电脑
---
...
#### 建立网络连接（TCP / UDP）
1. 客户端-> 服务端 SYN=1,ACK=0,Seq=x,Ack=0; 状态 = SYN_SEND
2. 服务端-> 客户端 SYN=1,ACK=1,Seq=y,Ack=x+1; 状态 = SYN_RECV
3. 客户端-> 服务端 ACK=1,SYN=0,Seq=x+1,Ack=y+1; 状态 = ESTABLISHED
...
TLS握手过程：
1. 客户端发送 ClientHello，包含支持算法和随机数。
2. 服务器返回 ServerHello、证书、公钥和服务器随机数。
3. 客户端验证证书。
4. 客户端生成 Pre-Master Secret，用服务器公钥加密发送。
5. 双方用三个随机数生成对称密钥。
6. 发送 Finished，进入加密通信。

- 非对称加密生成会话密钥，会话密钥作为对称加密的密钥进行通信；
---

#### 发起HTTP请求
- 请求行：`<请求方法> <请求URL> <HTTP版本>`
- 请求头：`Host`,`User-Agent`,`Accept`,`Accept-Encoding`,`Referer`,`Cookie`,`Cacha-Control`,`If-Modified-Since`,`If-None-Match`等；
- 请求体：POST请求有请求体，GET请求没有请求体；`axios` 请求体默认是 `application/json` 格式；`application/x-www-form-urlencoded`；`multipart/form-data`格式；
- 传输层：TCP在传输层将数据分割成多个数据段，每个数据段都有自己的序列号和确认号；
网络层：增加IP头，源IP地址和目的IP地址；
链路层：增加MAC头，源MAC地址和目的MAC地址；
物理层：网卡发送数据包；
路由器接收数据包：路由器解封装MAC地址，根据IP地址获取下一跳MAC地址，生成新的源MAC地址和目的MAC地址，数据包转发到下一跳；
---

#### 服务器处理请求
找到对应的IP地址的服务器后，服务器接收数据包：服务器解封装IP地址，根据端口找到对应的进程，服务器进程处理请求；
1. 判断是否有换缓存，如果有，则返回304
2. 如果没有，生成对应的状态码，返回响应头和响应体；
---

#### 浏览器接收响应
1. 浏览器接收响应头和响应体，响应头：`Content-Type`,`Content-Length`,`Cache-Control`,`ETag`,`Last-Modified`等；
2. 如果是302，则重定向到新的URL；
3. 如果是304，则从缓存中获取数据；
4. 如果是200，则从响应体中获取数据；
5. 判断Content-Encoding是否是gzip或br，如果是，则解压缩；
---

#### 渲染流水线开始
1. 流式解析html，遇到 `link css` ，阻塞渲染，请求CSS文件，解析CSS文件，不阻塞HTML解析和DOM构建；
2. 遇到 script 标签，如果没有 `defer` 或 `async` ，则阻塞渲染，请求JS文件，下载完成后，执行JS文件，阻塞DOM构建；
3. async 的脚本，下载完成后，立即执行。执行顺序不固定，适合独立脚本，不依赖DOM，不依赖其他脚本。
4. defer 的脚本，下载完成后，等待HTML解析完成后，执行，阻塞DOM构建；适合加载 依赖DOM和其他脚本。
5. 遇到图片、字体、视频，则请求资源，加载资源,加载完成后，替换或绘制；
---

#### 布局（Reflow）与绘制（Paint）
1. 布局（Reflow）：基于渲染树计算每个节点的几何尺寸和位置（宽高、margin、padding、坐标）。这是昂贵的操作，DOM 或样式的改变可能触发全树或部分树重算；
2. 绘制（Paint）：将每个节点的可视内容（颜色、文本、图像、阴影等）栅格化为图层像素；
3. 合成（Compositing）：现代浏览器把页面分成图层（layers），GPU 将各层合成（合并）到屏幕上，支持硬件加速（如动画时避免重绘整个页面，只移动图层）；
4. 首次内容渲染（First Paint / First Contentful Paint）和首次有意义渲染（First Meaningful Paint）由这些步骤产生并被性能监测 API（如 PerformanceTiming）记录；
---


### 浏览器渲染流程中的关键阶段有哪些？
1. 处理 HTML，构建 DOM
浏览器收到 HTML 字节流，边下载边解析成 tokens，然后转成 DOM 节点，最终构建出 DOM Tree。
2. 处理 CSS，构建 CSSOM
浏览器解析所有 CSS（内联、外链、style 标签），生成 CSSOM（CSS Object Model）。
3. 构建 Render Tree（渲染树）
DOM + CSSOM → 生成渲染树，只包含可见内容。
4. Layout（回流/Reflow）计算布局
浏览器根据 Render Tree 计算每个元素的几何信息（位置、大小等），这个过程可能触发子元素或父元素的重新布局。
触发回流的常见操作：修改 width/height、修改 display、获取布局信息 (offsetTop, clientHeight 会强制同步 layout)
5. Paint（重绘/Repaint）绘制
把元素的背景、文本、边框、阴影、图片等绘制成位图。
常见触发 repaint 的情况：
改变 color、background、border-radius、visibility、outline 等。
6. 分层（Layer）
浏览器会把页面拆成多个图层，类似 PS 图层。
触发新图层的操作：position: absolute + transform、position: fixed、transform, opacity（GPU 合成属性）、`<video>`、`<canvas>`、`<iframe>`等。
7. 合成（Composite）
GPU 将每个图层按顺序叠加、混合，最终合成一帧内容呈现到屏幕。
改变 transform: translate 或 opacity 只触发 合成，不需要回流和重绘 → 极高性能。
---
